...
static qboolean ParseStage (shaderStage_t *stage, char **text)
{
	...
	while (1)
	{
		...
		else if (!Q_stricmp (token, "map"))
		{
			...
			{
				stage->bundle.image[0] = R_FindImageFile (token, !shader.noMipMaps, !shader.noPicMip, D3DTADDRESS_WRAP);
		...
		else if (!Q_stricmp (token, "clampmap"))
		{
			...
			stage->bundle.image[0] = R_FindImageFile (token, !shader.noMipMaps, !shader.noPicMip, D3DTADDRESS_CLAMP);
			if (!stage->bundle.image[0])
			{
				ri.Printf (PRINT_WARNING, "WARNING: R_FindImageFile could not find '%s' in shader '%s'\n", token, shader.name);
				return qfalse;
		...
		else if (!Q_stricmp (token, "animMap"))
		{
			...
			while (1)
			{
				...
				{
					stage->bundle.image[num] = R_FindImageFile (token, !shader.noMipMaps, !shader.noPicMip, D3DTADDRESS_WRAP);
...
static void ParseSkyParms (char **text)
{
	...
	if (strcmp (token, "-"))
	{
		for (i = 0; i < 6; i++)
		{
			...
			shader.sky.outerbox[i] = R_FindImageFile ((char *) pathname, qtrue, qtrue, D3DTADDRESS_CLAMP);
	...
	if (strcmp (token, "-"))
	{
		...
		{
			Com_sprintf (pathname, sizeof (pathname), "%s_%s.tga", token, suf[i]);
			shader.sky.innerbox[i] = R_FindImageFile ((char *) pathname, qtrue, qtrue, D3DTADDRESS_CLAMP);
...
static void ComputeStageIteratorFunc (void)
{
	...
	else shader.optimalStageIteratorFunc = RB_StageIteratorGeneric;
}
	...
	*newShader = shader;
	...
	for (i = 0; i < newShader->numUnfoggedPasses; i++)
	{
		...
		*newShader->stages[i] = stages[i];
		...
		newShader->stages[i]->bundle.texMods = ri.Hunk_Alloc (size, h_low);
	...
	R_CreateHLSLProgramFromShader (newShader);
...
shader_t *R_FindShader (const char *name, int lightmapIndex, qboolean mipRawImage)
{
	...
	image = R_FindImageFile (fileName, mipRawImage, mipRawImage, mipRawImage ? D3DTADDRESS_WRAP : D3DTADDRESS_CLAMP);
	...
	else
	{
		...
		stages[0].bundle.isLightmap = qtrue;
		stages[0].active = qtrue;
...
static qhandle_t RE_RegisterShaderInternal (const char *name, int lightmapIndex, qboolean mipRawImage)
{
	...
	shader_t *sh = R_FindShader (name, lightmapIndex, mipRawImage);
...
{
	return RE_RegisterShaderInternal (name, lightmapIndex, qtrue);
}
...
void R_ShaderList_f (void)
{
	...
	for (i = 0; i < tr.numShaders; i++)
	{
		...
		else shader = tr.shaders[i];

		ri.Printf (PRINT_ALL, "%i ", shader->numUnfoggedPasses);

		if (shader->lightmapIndex >= 0)
			ri.Printf (PRINT_ALL, "L ");
		else ri.Printf (PRINT_ALL, "  ");
...
static void CreateInternalShaders (void)
{
	...
	shader.lightmapIndex = LIGHTMAP_NONE;
	stages[0].bundle.image[0] = tr.fogImage;
	stages[0].active = qtrue;
	stages[0].stateBits = GLS_DEFAULT;
	stages[0].rgbGen = CGEN_FOG;
	stages[0].alphaGen = AGEN_SKIP;
	stages[0].bundle.tcGen = TCGEN_FOG;
